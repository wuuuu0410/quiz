...
public class FillinServiceImpl implements FillinService {
	...
	public BasicRes fillin(FillinReq req) {
		...
		try {
			...
			for (Question item : quList) {// 123
				List<Fillin> fillinList = req.getFillinList();
				for (Fillin fillin : fillinList) {// 133
					// id 不一致，跳過
					if (item.getId() != fillin.getqId()) {
//						if (item.isNecessary() && fillinList.indexOf(fillin) == fillinList.size() - 1) {
...
//						}
					...
					// 上面的程式碼只有對 question_id和anwer檢查，所以其餘的屬性內容可能是不合法的
					// 直接使用 Question item 的值是因為這些值都是從DB來，當初已有檢察查過
	...
	public FeedbackRes feedback(FeedbackReq req) {
		Optional<Quiz> op = quizDao.findById(req.getQuizId());
		if (op.isEmpty()) {
			return new FeedbackRes(ResMessage.QUIZ_ID_NOT_FOUND.getCode(), //
			...
			try {
				...
				for (Fillin fillin : fillinList) {
					...
					for (String option : optionArray) {
						...
						// 計算該選項出現的次數
						// 原本字串的長度-被取代後字串的長度，要除以選項的長度才會是真正的次數
						...
						//把有累加次數的optionCountMap 覆蓋回 countMap 中(相同的題號)
